...
public class PersonalMessageFragment extends Fragment implements IMessageView,OnRefreshListener,OnLoadmoreListener {
    ...
    private List<MsgResult.MsgDataBean.MsgInfoBean> msgInfoBeans /*= new ArrayList<>()*/;
    private NoticeRecyclerViewAdapter adapter;
    /**
     * 个人消息
     */
    public static final int PERSONAL_MSG = 1;
    private String Tag = "PersonalMessageFragment_";

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {
        view = inflater.inflate(R.layout.fragment_personal_message_, null);
    ...
    private void loadData() {
        ...
        messagePresenter.loadAreaNoticeList(0,1,pageSize,PERSONAL_MSG);
    }
    @Override
    ...
    private void hideMenu(){
        //把列表item点击事件设置为非编辑状态，即可点击跳转到消息详情页面
    ...
    private void showMenu(){
        //把列表item点击事件设置为编辑状态，即可点击选择删除指定消息
    ...
    @Override
    public void showToastNoData() {//没有更多数据
        MyToast.showToast(getActivity(),R.string.no_per_data);
        smartRefreshPerMsg.finishLoadmore();
    }
    ...
    @Override
    public void onRefresh(RefreshLayout refreshlayout) {//SmartRefreshView 下拉刷新回调这个方法
        currentPage = 1;
        messagePresenter.loadAreaNoticeList(1,currentPage,pageSize,PERSONAL_MSG);
    }

    @Override
    public void onLoadmore(RefreshLayout refreshlayout) {//SmartRefreshView 上拉加载回调这个方法
        if(msgInfoBeans.size() >= pageSize){
            currentPage ++;
        }
        MyLog.print(Tag,"currentPage:"+currentPage+"    msgInfoBeans.size()"+msgInfoBeans.size(),MyLog.PRINT_RED);
        messagePresenter.loadMore(currentPage,pageSize,PERSONAL_MSG);
        smartRefreshPerMsg.finishLoadmore();
    }
    @Override
    public void showToastNoRefreshData() {//没有刷新数据
